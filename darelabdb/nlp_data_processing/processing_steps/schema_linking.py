...
def get_value_links(
    data_point: SqlQueryDatapoint,
    column_names_field: str = "column_names",
    table_names_field: str = "table_names",
) -> SqlQueryDatapoint:
    # Get table and column names
    table_names = data_point.db_schema[table_names_field]
    column_names = data_point.db_schema[column_names_field]

    table_to_col = {
        table_name: [
            column_name
            for col_table_idx, column_name in column_names
    ...
    value_links = {}
    for table_name, table_columns in table_to_col.items():
        cur_table_links = {}
        for column_name in table_columns:
            # Find schema links for the current column
            cur_column_links = get_database_matches(
                data_point.nl_query,
                table_name,
                column_name,
                data_point.db_path,
            )
            if len(cur_column_links) > 0:
                cur_table_links[column_name] = cur_column_links

        # Add schema links for this table only if any where found
        if len(cur_table_links) > 0:
            value_links[table_name] = cur_table_links

    data_point.value_links = value_links
